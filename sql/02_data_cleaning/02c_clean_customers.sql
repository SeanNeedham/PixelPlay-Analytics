...
*/
-- standardise customer fields
WITH   cleaned_customers
AS     (SELECT NULLIF (LOWER(TRIM(USER_ID)), '') AS user_id,
...
-- rank duplicate customer records so the latest record can be kept
       ranked_customers
AS     (SELECT user_id,
               age,
               age_band,
        ...
        -- exclude records without a usable customer id
        WHERE  user_id IS NOT NULL)
SELECT -- create final cleaned customer table
       user_id,
       age,
       age_band,
       gender,
       ...
       source_quality_flag,
       -- data quality status
       CASE WHEN age IS NULL THEN 'Missing Age' WHEN country_code = 'UNKNOWN' THEN 'Missing Country' WHEN signup_date IS NULL THEN 'Missing Signup Date' WHEN loyalty_tier = 'Unknown' THEN 'Missing Loyalty Tier' WHEN email_opt_in = 'Unknown' THEN 'Missing Email Preference' ELSE 'Complete' END AS data_quality_status
